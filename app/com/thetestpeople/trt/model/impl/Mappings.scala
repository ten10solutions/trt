...
trait Mappings { self: SlickDao ⇒
  ...
  class AnalysisMapping(tag: Tag) extends Table[Analysis](tag, "analysis") {
    ...
    def lastSummary = column[Option[String]]("last_summary")
    
    def * = (testId, configuration, status, weather, consecutiveFailures, failingSince, lastPassedExecutionId, lastPassedTime,
      lastFailedExecutionId, lastFailedTime, whenAnalysed, medianDuration, lastSummary) <> (Analysis.tupled, Analysis.unapply)
    ...
    def ? = (testId.?, configuration.?, status.?, weather.?, consecutiveFailures.?, failingSince, lastPassedExecutionId,
      lastPassedTime, lastFailedExecutionId, lastFailedTime, whenAnalysed.?, medianDuration, lastSummary).shaped <> ({ r ⇒
        import r._; _1.map(_ ⇒
          Analysis.tupled((_1.get, _2.get, _3.get, _4.get, _5.get, _6, _7, _8, _9, _10, _11.get, _12, _13)))
  ...
  class TestMapping(tag: Tag) extends Table[Test](tag, "tests") {
     ...
     * (Autogenerated) helper code for outer joins
     */
    def ? = (id.?, name.?, group, deleted.?).shaped <> ({ r ⇒
      import r._; _1.map(_ ⇒
        Test.tupled((_1.get, _2.get, _3, _4.get)))
    }, (_: Any) ⇒ throw new Exception("Inserting into ? projection not supported."))
  ...
  class IgnoredTestConfigurationMapping(tag: Tag) extends Table[IgnoredTestConfiguration](tag, "ignored_test_configurations") {
     ...
     * (Autogenerated) helper code for outer joins
     */
    def ? = (testId.?, configuration.?).shaped <> ({ r ⇒
      import r._; _1.map(_ ⇒ IgnoredTestConfiguration.tupled((_1.get, _2.get)))
    }, (_: Any) ⇒ throw new Exception("Inserting into ? projection not supported."))

...
class ServiceTest extends FlatSpec with ShouldMatchers {
  ...
  it should "allow you to fetch a batch and its executions" in {
    ...
    val Some(BatchAndExecutions(batch, List(execution), Some(log))) = service.getBatchAndExecutions(batchId)
  ...
  it should "allow you to fetch a test and its executions, including analysis" in {
    ...
    val Some(TestAndExecutions(testAndAnalysis, List(execution))) = service.getTestAndExecutions(testId)
  ...
  case class Setup(service: Service)
